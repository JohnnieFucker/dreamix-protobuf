...
function encodeTag(type, tag) {
    ...
    return codec.encodeUInt32((tag << 3) | value); // eslint-disable-line
}
...
function checkMsg(msg, protos) {
    ...
    for (const name in protos) {
        if (protos.hasOwnProperty(name)) {
            ...
            case 'optional' :    // eslint-disable-line
                if (typeof (msg[name]) !== 'undefined') {
...
function encodeProp(value, type, offset, buffer, protos) {
    ...
    case 'string':
        ...
        break;
    default :
        message = protos.__messages[type] || Encoder.protos[`message ${type}`];
        if (message) {
            // Use a tmp buffer to build an internal msg
            const tmpBuffer = new Buffer(Buffer.byteLength(JSON.stringify(value)) * 2);
            length = 0;

            length = encodeMsg(tmpBuffer, length, message, value);    // eslint-disable-line
            // Encode length
            offset = writeBytes(buffer, offset, codec.encodeUInt32(length));
            // contact the object
...
Encoder.encode = (route, msg) => {
    ...
    const length = Buffer.byteLength(JSON.stringify(msg)) * 2;
